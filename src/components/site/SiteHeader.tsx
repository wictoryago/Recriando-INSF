  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 py-3">
      <nav
        ...
        className={cn(
          "nav-panel mx-auto flex max-w-6xl items-center justify-between rounded-full px-4 py-2.5 transition-all duration-300 sm:px-6",
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {navLinks.map((link) => (
            <Link
              ...
              className="rounded-full px-3.5 py-2 text-sm font-medium text-foreground/90 transition-colors hover:text-foreground"
            >
              {link.label}
            </Link>
          ))}
      ...
      {open && (
        <div className="nav-panel mx-auto mt-2 max-w-6xl rounded-3xl p-4 lg:hidden">
          <div className="flex flex-col">
            {navLinks.map((link) => (
              <Link
                ...
                activeOptions={{ exact: link.to === "/" }}
                activeProps={{ className: "text-gold" }}
                className="rounded-xl px-3 py-3 text-sm font-medium text-foreground/90 transition-colors hover:bg-secondary hover:text-foreground"
              >
                {link.label}
              </Link>
            ))}
